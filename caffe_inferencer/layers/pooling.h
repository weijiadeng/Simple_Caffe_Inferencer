...
inline Tensor * pool(Tensor * tensor_img, int kernel_size, int stride, void(*fp)(POOL_HELPER_FUNC_PARAM)) {
    ...
    infer_conv_shape(tensor_img, result_dim, kernel_size, kernel_size, tensor_img->shape[2], 0, stride);
    ...
    for (input_pos[3] = 0, output_pos[3] = 0; input_pos[3] < tensor_img->shape[3]; ++input_pos[3], ++output_pos[3])
        ...
        for (input_pos[2] = 0, output_pos[2] = 0; input_pos[2] < tensor_img->shape[2]; ++input_pos[2], ++output_pos[2])
            //Each Y axis
            for (input_pos[1] = 0, output_pos[1] = 0; input_pos[1] + kernel_size <= tensor_img->shape[1]; input_pos[1] += stride, ++output_pos[1])
                //Each X axis
                for (input_pos[0] = 0, output_pos[0] = 0; input_pos[0] + kernel_size <= tensor_img->shape[0]; input_pos[0] += stride, ++output_pos[0])
                {
